...
import { getSuperspecifications } from "../services/superspecificationService.js";

// const createSubCategoryC = async (req, res) => {
//   try {
//     const categoryExists = await getSuperspecificationById(req.body.categoryId);
//     if (!categoryExists) {
...
const getSuperspecificationsC = async (req, res) => {
  try {
    const superspecifications = await getSuperspecifications();
    if (!superspecifications) {
      return response.sendError(res, 400, 2003);
    }
    return response.sendSuccess(
      res,
      200,
      "Superspecification retrieved",
      superspecifications
    );
  } catch (error) {
    console.error("Error retrieving superspecification:", error);
